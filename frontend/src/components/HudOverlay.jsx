...
export default function HudOverlay({ pro }) {
  ...
  return (
    <>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-40 border-t border-nerv-orange/60 bg-background/90 backdrop-blur-sm text-[10px] tracking-widest font-mono">
        <div className="flex items-center justify-between px-3 md:px-6 py-1">
          <div className="flex items-center gap-3 md:gap-5 overflow-hidden">
            <span className="text-nerv-orange">● REC</span>
            <span className="hidden sm:inline text-foreground/70">SYNC {synch}%</span>
            <span className="hidden md:inline text-foreground/70">AT-FIELD {at}%</span>
            <span className="hidden md:inline text-foreground/70">PULSE {pulse}</span>
            <span className="hidden lg:inline text-foreground/70">GEOFRONT // TOKYO-3</span>
          </div>
          <div className="flex items-center gap-3 text-foreground/80">
            <span className="hidden md:inline">{time.toUTCString().slice(5, 25)} UTC</span>
            <span className="text-nerv-green">{time.toLocaleTimeString("en-GB")}</span>
          </div>
        </div>
        {!pro && (
          <div className="overflow-hidden border-t border-nerv-orange/40 h-5">
            <div className="animate-marquee whitespace-nowrap text-nerv-orange/80 leading-5 px-3">
